(function(){
const roles={
 'job-api-backend-developer.html':'API / Backend Developer',
 'job-frontend-developer.html':'Frontend Developer',
 'job-digital-marketing.html':'Digital / Marketing Specialist',
 'job-future-roles.html':'Future Roles / General Application'
};
...
const reviewUrl=code=>'job/index.html?code='+encodeURIComponent(code);
 ...
 form.addEventListener('submit',async function(e){
  ...
  try{
   ...
   source.forEach((value,key)=>{
    ...
    if(key==='project')data.append('motivation',String(value));
    else if(key==='experience'&&role==='Future Roles / General Application')data.append('motivation',String(value));
    else if(key==='agreement')return;
   ...
   try{sessionStorage.setItem('jobApplication',JSON.stringify(payload.application||{}))}catch{}
   form.reset();
