...
async function handle(form,e){
 ...
 const cv=form.querySelector('input[name="cv"]');if(!cv||!cv.files.length){if(cv)cv.reportValidity();return}
 ...
 try{
  ...
  const copy=result.querySelector('.copy-code-btn');if(copy)copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(code);copy.innerHTML='<i class="fa-solid fa-check"></i> Copied'}catch{}});
...
document.addEventListener('submit',e=>{const form=e.target;if(form&&form.tagName==='FORM')handle(form,e)},true);
})();